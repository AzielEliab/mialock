...
/**
 * M.I.A.Lock download tracker (Cloudflare Worker).
 *
 * GET  /download?repo=AzielEliab/mialock&tag=latest&asset=...
 ...
 * Isolated counter: Worker mialock-download-tracker, project mialock.
...
const GITHUB_RELEASES = "https://github.com/AzielEliab/mialock/releases";
const GITHUB_LATEST = "https://github.com/AzielEliab/mialock/releases/latest";
const HOST = "https://mialock-download-tracker.vibelock.workers.dev";
const GITHUB_REPO = "https://github.com/AzielEliab/mialock";
const INSTALL_LINE = "curl -fsSL https://mialock-download-tracker.vibelock.workers.dev/install.sh | bash";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 MIALock-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/mialock", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/mialock/releases/latest", { headers });
    if (relRes.ok) {
...
# M.I.A.Lock one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
WORKDIR="\${MIALOCK_HOME:-\$HOME/mialock}"
mkdir -p "\$WORKDIR"
cd "\$WORKDIR"
echo "Downloading counted tarball from \${HOST}/download (User-Agent Mozilla/5.0)…"
curl -fsSL -A 'Mozilla/5.0' "\${HOST}/download?asset=\${ASSET}" -o "\${ASSET}"
tar -xzf "\${ASSET}"
DIR="\$(find . -maxdepth 1 -type d -name 'mialock-*' | head -n 1)"
if [ -n "\${DIR}" ]; then
  cd "\${DIR}"
fi
python3 -m venv .venv
...
echo
echo "Installed M.I.A.Lock."
echo "Run:  mialock map"
...
<title>M.I.A.Lock — Aziel Eliab</title>
<meta name="description" content="Purpose-bound missing-person event map and archive/Doe search options by Aziel Eliab. Historical pins only. Doe leads ≠ ID.">
<meta name="author" content="Aziel Eliab">
<link rel="canonical" href="https://mialock-download-tracker.vibelock.workers.dev/">
<meta property="og:title" content="M.I.A.Lock — Aziel Eliab">
<meta property="og:description" content="Purpose-bound missing-person event map and archive/Doe search options by Aziel Eliab. Historical pins only. Doe leads ≠ ID.">
<meta property="og:url" content="https://mialock-download-tracker.vibelock.workers.dev/">
...
<script type="application/ld+json">
{
  ...
  "codeRepository": "https://github.com/AzielEliab/mialock",
  "downloadUrl": "https://mialock-download-tracker.vibelock.workers.dev/download",
  "license": "https://www.apache.org/licenses/LICENSE-2.0",
  "url": "https://mialock-download-tracker.vibelock.workers.dev/",
  "description": "Purpose-bound missing-person event map and archive/Doe search options by Aziel Eliab. Historical pins only. Doe leads ≠ ID."
}
</script>
<!-- gitbaby-seo -->
<style>
...
<h1>M.I.A.Lock</h1>
  ...
  <div id="meshStrip" aria-label="Suite Live Nodes">
    ...
    <div>
      ...
      <button id="meshJoin" type="button" title="Join as mialock. Refused while mesh is OFF. No auto-join.">Join</button>
  ...
  <div class="card">
    ...
    <script>
      (function () {
        var cmd = "curl -fsSL https://mialock-download-tracker.vibelock.workers.dev/install.sh | bash";
        var btn = document.getElementById("install-btn");
        var pre = document.getElementById("install-cmd");
        if (!btn) return;
        btn.addEventListener("click", function () {
          function done(ok) {
            btn.textContent = ok ? "Copied! Paste in Terminal, then run mialock map" : "Select the command, copy it, then run mialock map";
      ...
      (function () {
        ...
        function paintMesh(raw) {
          ...
          $("meshProducts").textContent = "Catalog MCP mesh_* · FragGate slug=mesh · /v1/mesh/* PROXY · QNS-CD-1.0 photon QNS1 cross-map · not AnonBroadcast · not AZMail ring · not a Node Gate · no public qnsd proxy" + extra;
        }
        ...
        async function refreshMesh() {
          try {
            ...
            paintMesh(merged);
            var nodeId = sessionStorage.getItem("mialock_mesh_node");
            if (on && nodeId) {
        ...
        $("meshDisable").onclick = async function () {
          sessionStorage.removeItem("mialock_mesh_node");
          paintMesh(await meshPost("/v1/mesh/disable", {}));
          refreshMesh();
        };
        $("meshJoin").onclick = async function () {
          var j = await meshPost("/v1/mesh/join", { product: "mialock", label: "M.I.A.Lock Worker" });
          var inner = unwrapMesh(j);
          var id = inner.node_id || inner.id || (inner.session && inner.session.node_id);
          if (id) sessionStorage.setItem("mialock_mesh_node", String(id));
          paintMesh(j);
          refreshMesh();
        };
        $("meshLeave").onclick = async function () {
          var id = sessionStorage.getItem("mialock_mesh_node");
  ...
  async fetch(request, env) {
    ...
    if ((url.pathname === "/llms.txt" || url.pathname === "/llms.txt/" || url.pathname === "/ai.txt" || url.pathname === "/ai.txt/") && request.method === "GET") {
      const body = [
        ...
        "",
        "Author: Aziel Eliab",
        "One-line: " + "Purpose-bound missing-person event map and archive/Doe search options by Aziel Eliab. Historical pins only. Doe leads ≠ ID.",
        "GitHub: " + GITHUB_REPO,
    ...
    if ((url.pathname === "/cite.json" || url.pathname === "/cite.json/") && request.method === "GET") {
      return json({"author": "Aziel Eliab", "title": "M.I.A.Lock", "github": "https://github.com/AzielEliab/mialock", "download": "https://mialock-download-tracker.vibelock.workers.dev/download", "doi": null, "license": "Apache-2.0", "catalog": "https://aziel-runtime.vibelock.workers.dev/", "one_line": "Purpose-bound missing-person event map and archive/Doe search options by Aziel Eliab. Historical pins only. Doe leads ≠ ID."});
    }
    // /gitbaby-seo-routes
